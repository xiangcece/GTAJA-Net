...
class Regressor(nn.Module):
    def __init__(self, smpl_mean_params=SMPL_MEAN_PARAMS):
        ...
        self.fc11 = nn.Linear(512 * 4 + npose + 13, 1024)
        self.drop11 = nn.Dropout()
        self.fc22 = nn.Linear(1024, 1024)
        self.drop22 = nn.Dropout()
    ...
    def forward(self, x, init_pose=None, init_shape=None, init_cam=None, n_iter=3, is_train=False, J_regressor=None):
        ...
        for i in range(n_iter):             
            ...
            xc = self.fc22(xc)
            xc = self.drop22(xc)
            pred_pose = self.decpose1(xc) + pred_pose #[96, 144])
        ...
        for ancestor_idx, reg in zip(ANCESTOR_INDEX, self.joint_regs):
            ances = torch.cat([xc] + [pose[j] for j in ancestor_idx], dim=1)
            pose.append(reg(ances))                   
        pred_pose = torch.cat(pose, dim=1) + pred_pose#[512, 144])
        #print(pred_pose.shape) #[96, 144]
        for i in range(n_iter):
            xc = torch.cat([x, pred_pose, pred_shape, pred_cam], 1)
            xc = self.fc1(xc)
            xc = self.drop1(xc)
        ...
        pred_rotmat = rot6d_to_rotmat(pred_pose).view(batch_size, 24, 3, 3) #[96, 24, 3, 3]
