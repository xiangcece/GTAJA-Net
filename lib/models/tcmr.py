...
class TemporalEncoder(nn.Module):
    def __init__(
        ...
        self.attention_gafi = TemporalAttention(attention_size=2048, seq_len=16, non_linearity='tanh')
    
    def forward(self, x, is_train=False):
        # NTF -> TNF
        scores_gafi = self.attention_gafi(x)
        out_gafi = torch.mul(x, scores_gafi[:, :, None])
        out_gafi= torch.sum(out_gafi, dim=1)  # N x 2048
        ...
        y = torch.cat((y_bef[:, None, :], y_cur[:, None, :], y_aft[:, None, :]), dim=1) #[32, 3, 2048]
        
       
        scores = self.attention(y)
        out = torch.mul(y, scores[:, :, None])
        out = torch.sum(out, dim=1) + out_gafi  # N x 2048
